...
# Store product catalog separately for easy updates
PRODUCT_CATALOG = {
    # ... [Your existing PRODUCT_CATALOG dictionary] ...
}

# Store information for Tariq Halal Meats
STORE_INFO = """
# ... [Your existing STORE_INFO string] ...
"""

def find_product(search_term):
    """Helper function to search products in catalog"""
        ...
        for product, price in products.items():
            if search_term in product.lower():
                if category not in results:
                    results[category] = []
                results[category].append((product, price))
    return results

def generate_response(user_input):
    """Generate response either from product catalog or OpenAI"""
    # First check for direct product matches
    product_results = find_product(user_input)
    if product_results:
    ...
    # Fall back to AI for other queries
    try:
        prompt = f"""
        ...
        return "Sorry, I'm having trouble answering that. Please call ☎️ 0208 908 9440 for assistance."
...
def whatsapp_webhook():
    try:
        ...
        logger.info(f"Incoming request data: {request.values}")
        ...
        logger.info(f"Processing message: {incoming_msg}")
...
@app.route("/")
def home():
    return "✅ Tariq Halal Meats WhatsApp Bot is running"
